#!/usr/bin/env python3
"""
Search Tool Evaluation - Initialize MultiAgentRunner

Initializes MultiAgentRunner the same way backend does, then runs evaluation.

Usage (in Docker):
    docker exec sujbot_backend python scripts/evaluate_with_runner.py
"""
...
OUTPUT_PATH = "/app/results/eval_hyde_expand2_k100_FIXED.json"
K = 100
USE_HYDE = True
NUM_EXPANDS = 2
...
async def main():
    ...
    # Load config (same as backend)
    print("Loading configuration...")
    config_path = Path("/app/config.json")
    with open(config_path) as f:
        full_config = json.load(f)

    # Build runner config (same as AgentAdapter)
    runner_config = {
        "api_keys": {
    ...
    # Get tool registry (global singleton)
    ...
    # Check vector store configuration (multi-layer is always enabled if vector store was built with it)
    vector_store_has_multi_layer = True  # Multi-layer is built into the vector store structure
    ...
    print(f"  enable_graph_boost={ENABLE_GRAPH_BOOST}")
    print(f"  reranker_available={has_reranker}")
    print(f"  multi_layer_retrieval={vector_store_has_multi_layer}")
    print(f"\nAggregate Metrics:")
