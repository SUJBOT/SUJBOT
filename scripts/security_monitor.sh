...
#
# Usage:
#   ./scripts/security_monitor.sh           # Regular 4h check
...
CONTAINERS=$(jq -r '.security_monitoring.checks.docker_health.containers | join(" ")' "$CONFIG_FILE")
...
if command -v curl &> /dev/null; then
    HEALTH_STATUS=$(curl -s -o /dev/null -w "%{http_code}" --connect-timeout 5 http://localhost:8000/health 2>&1) || HEALTH_STATUS="000"
    if [ "$HEALTH_STATUS" = "200" ]; then
        HEALTH_RESPONSE=$(curl -s --connect-timeout 5 http://localhost:8000/health 2>&1) || HEALTH_RESPONSE="{}"
    fi
fi
