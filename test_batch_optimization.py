...
def test_parallel_mode(num_sections: int = 30):
    ...
    config = SummarizationConfig(
        enable_prompt_batching=False,  # Disable batching
        max_workers=20
    )
    ...
    logger.info(f"📊 Average: {elapsed_time / len(summaries):.3f} seconds per summary")
...
def test_batching_mode(num_sections: int = 30, batch_size: int = 15):
    ...
    config = SummarizationConfig(
        enable_prompt_batching=True,  # Enable batching
...
def main():
    ...
    try:
        ...
        if speedup >= 10:
            logger.info("🎉 EXCELLENT! Achieved 10×+ speedup")
        elif speedup >= 5:
            logger.info("✅ GOOD! Achieved 5-10× speedup")
        elif speedup >= 2:
            logger.info("👍 OK! Achieved 2-5× speedup")
        else:
            logger.warning("⚠️  Speedup lower than expected")
