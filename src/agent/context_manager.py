...
from response.usage.input_tokens (available from all providers, zero extra latency):

    Layer 1 (70-85%): Tool output pruning — replace old tool_result content with
        compact placeholders, strip base64 images. No LLM call.
    Layer 2 (85-95%): LLM summary compaction — one LLM call to summarize older
        conversation history while preserving recent turns.
    Layer 3 (>95%):   Emergency truncation — existing naive message slice.
...
class ContextBudgetMonitor:
    ...
    def check(self) -> ContextBudget:
        ...
        return ContextBudget(
            used_tokens=self._used_tokens, max_tokens=self.context_window
        )
...
def prune_tool_outputs(
    ...
    Args:
        ...
        protect_last_n: Number of trailing user messages to leave untouched.
...
def _load_compaction_prompt() -> str:
    ...
    logger.warning("Compaction prompt not found at %s, using inline fallback", _COMPACTION_PROMPT_PATH)
    return (
...
def compact_with_summary(
    ...
    Keeps the first user message and the last ``protect_last_n`` message pairs
    (assistant + user) intact.  Everything in between is replaced by a single
    assistant message containing the LLM-generated summary.
    ...
    Args:
        ...
        system_prompt: System prompt (str or list-of-dicts for caching).
        protect_last_n: Number of trailing message pairs to preserve.

    Returns:
        Compacted message list (or original on failure).
    """
    # Need at least first message + something to summarize + protected tail
    min_messages = 1 + protect_last_n * 2 + 2
    if len(messages) < min_messages:
        logger.debug("Not enough messages (%d) for Layer 2 compaction", len(messages))
        return messages

    first_msg = messages[0]
    tail_count = protect_last_n * 2  # pairs of (assistant, user)
    ...
    try:
        ...
        logger.warning("Layer 2 compaction LLM call failed: %s — returning messages unchanged", e)
        return messages
...
def emergency_truncate(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """
    Layer 3: Keep first message + last 6 messages.

    This is the existing fallback logic, preserved as the last resort.
    """
    max_history = 7  # 1 original + 6 recent
    if len(messages) <= max_history:
        return messages

    truncated_count = len(messages) - max_history
